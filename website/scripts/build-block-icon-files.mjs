...
const PRIMARY_DIR = path.join(PUBLIC_ICON_ROOT, "primary");
const UNUSED_DIR = path.join(PUBLIC_ICON_ROOT, "unused");
...
async function resolveSource(blockEntry) {
  ...
  const candidateBlockIds = (() => {
    ...
    if (primaryBlockId === "end_portal") out.push("end_portal_frame");
...
async function main() {
  ...
  await fs.rm(PRIMARY_DIR, { recursive: true, force: true });
  ...
  for (const block of blocks) {
    ...
    const dst = path.join(PRIMARY_DIR, `${toBlockIconKey(block)}.png`);
    if (!src) {
      missing.push(block);
      continue;
    }
  ...
  const worldBorderDst = path.join(PRIMARY_DIR, "world_border.png");
  if (await fileExists(worldBorderSrc)) {
    await fs.copyFile(worldBorderSrc, worldBorderDst);
  }

  const reportPath = path.join(ROOT, "reports", "block-icons", "primary-missing.txt");
  await fs.mkdir(path.dirname(reportPath), { recursive: true });
  await fs.writeFile(reportPath, `${missing.join("\n")}\n`, "utf8");

  console.log(`Primary icon entries: ${blocks.length}`);
  console.log(`Copied: ${copied}`);
  console.log(`Unused (excluded) copied: ${unusedCopied}`);
  console.log(`Missing: ${missing.length}`);
  console.log(`Primary dir: ${path.relative(ROOT, PRIMARY_DIR)}`);
  console.log(`Unused dir: ${path.relative(ROOT, UNUSED_DIR)}`);
